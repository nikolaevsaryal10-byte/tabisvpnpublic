...
object SettingsManager {
    ...
    fun isUsingHevTun(): Boolean {
        val pref = MmkvManager.decodeSettingsBool(AppConfig.PREF_USE_HEV_TUNNEL, false)
        if (!pref) return false
        val nativeDir = AngApplication.application.applicationInfo.nativeLibraryDir
        val hevLib = File(nativeDir, "libhev-socks5-tunnel.so")
        return hevLib.exists()
    }
